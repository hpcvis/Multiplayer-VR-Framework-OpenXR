...
//This was created by https://github.com/oshoham/UnityGoogleStreamingSpeechToText with modifications made to adjust to PUN and multiple script integration
//Refer to the github above for instructions on how to use
//The main difference is that this custom script allows us access to the streaming methods without creating it ourselves
//Therefore the only line that was added in to send the final transcript data to "STTData.cs" was in line 302
    ...
    private async Task HandleTranscriptionResponses()
    {
        while (await _streamingCall.ResponseStream.MoveNext(default))
        {
            ...
            if (result.IsFinal)
            {
                ...
                GameObject.FindGameObjectWithTag("Player").GetComponent<STTData>().sendText(transcript);
            }
            else
            {
